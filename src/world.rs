...
impl World {
    ...
    pub fn add_entity<B: Bundle>(&self, bundle: B) -> Result<Entity, WorldError> {
        if !self.entities.entities_left() {
            return Err(WorldError::TooManyEntitiesInWorld);
        }
        let archetype = self.archetypes.get(&B::archetype_id()).ok_or(WorldError::ArchetypeNotFound)?;
        if !archetype.entities_left() {
            return Err(WorldError::TooManyEntitiesInArchetype);
        }
        let entity = self.entities.alloc();
        let data = unsafe {
            archetype.add_entity(entity.id, bundle.into_component_ptrs().iter())
        };
        self.entities.push_location(&entity, EntityLocation {
